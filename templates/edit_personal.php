...
$count = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `users` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' ;'));
if ($count['COUNT(*)'] > 0) {
$content = mysqli_fetch_array(mysqli_query($db, 'SELECT * FROM `users` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1;'));
...
`thirdname` = \''.mysqli_real_escape_string($db, $_POST['thirdname']).'\',
...
mysqli_query($db, 'DELETE FROM `users_cats` WHERE `user_id` = '.$_GET['id'].' ;') or mysqli_error($db);
...
<head>
<meta charset=utf-8>
<title>Сотрудники - Панель управления</title>
   ...
   <div class="adm-form" style="padding-top:0;">
         ...
         <div class="item" >
            ...
            <div class="item">
              ...
              <div class="value">
                <input type="text" name="notif_email" value="<?=$content['email'];?>"  />
              </div>
            </div>
        ...
        <br><br>
            ...
            <ul>
              <?php
              $sql2 = mysqli_query($db, 'SELECT * FROM `cats_users` where `service` = 1 and `service_id` = 33 ;');
              if (mysqli_num_rows($sql2) != false) {
                    while ($row2 = mysqli_fetch_array($sql2)) {
                     $checked = (check_user_cats($_GET['id'], $row2['cat_id'], 1)) ? 'checked' : '';
                     echo '<li style="padding:5px 0px;    display: block;    text-align: left;"><label><input type="checkbox" name="cat_1[]" value="'.$row2['cat_id'].'" '.$checked.'/>'.$row2['name'].'</label></li>';
                    }
              }
            ...
            <ul>
              <?php
              $sql3 = mysqli_query($db, 'SELECT * FROM `cats_users` where `service` = 1 and `service_id` = 33 ;');
              if (mysqli_num_rows($sql3) != false) {
                    while ($row3 = mysqli_fetch_array($sql3)) {
                    $checked = (check_user_cats($_GET['id'], $row3['cat_id'], 2)) ? 'checked' : '';
                     echo '<li style="padding:5px 0px;    display: block;    text-align: left;"><label><input type="checkbox" name="cat_2[]" value="'.$row3['cat_id'].'" '.$checked.'/>'.$row3['name'].'</label></li>';
                    }
              }
