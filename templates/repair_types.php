...
$sql = mysqli_query($db, 'SELECT * FROM `repair_type` ;');
if (mysqli_num_rows($sql) != false) {
      ...
      <a class="t-3" title="Редактировать карточку" href="/edit-repair-type/'.$row['id'].'/" ></a>
      <a class="t-5" title="Удалить карточку" onclick=\'return confirm("Вы уверены, что хотите удалить #'.$row['id'].'?")\'  style="float:right" href="/del-repair-type/'.$row['id'].'/"></a></td></tr>';
      }
      } else {
...
<?=menu_dash();?>
  ...
  <div class="adm-catalog">
     ...
     <div class="add">
      <a style="width: auto;padding-left: 7px;padding-right: 7px;" href="/add-repair-type/" class="button">Добавить вид ремонта</a>
    </div>
     <br>
