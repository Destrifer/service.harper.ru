...
if (mysqli_num_rows($sql) != false) {
    while ($row = mysqli_fetch_array($sql)) {
          $document = create_or_get_document(\models\User::getData('id'), $row['id']);
          if ($document['deleted'] != 1) {

          if ($document['status'] == 'Нет') {
          $document['status'] = 'Вышлите оригинал';
          $style = 'background: rgba(255, 51, 0, 0);background-color: rgba(255, 51, 0, 0.14) !important;';

          } else {
          $document['status'] = 'Оригинал получен';

          $style = 'background: rgba(51, 204, 0, 0);background-color: rgba(51, 204, 0, 0.14) !important;';
          }

          $content_list .= '<tr style="'.$style.'">
  ...
  <table id="table_content" class="display" cellspacing="0" width="100%">
        <thead>
            <tr>
                ...
                <th align="left">Статус получения</th>
            </tr>
        </thead>
