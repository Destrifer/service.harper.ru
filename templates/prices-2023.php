...
$sql = mysqli_query($db, 'SELECT * FROM `cats` where `service` = 1;');
      while ($row = mysqli_fetch_array($sql)) {
        $count_prices = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `pricess-2023` where `cat_id` = '.$row['id']));
        if ($count_prices['COUNT(*)'] == 0) {
            mysqli_query($db, 'INSERT INTO `prices-2023` (
            `cat_id`
            ) VALUES (
...
$sql = mysqli_query($db, 'SELECT * FROM `prices-2023`;');
      while ($row = mysqli_fetch_array($sql)) {
        ...
        if ($count_service['COUNT(*)'] == 0) {
            mysqli_query($db, 'DELETE FROM `prices-2023` WHERE `id` = '.$row['id'].' LIMIT 1;') or mysqli_error($db);
        }
...
<head>
<meta charset=utf-8>
<title>Тарифы обслуживания 2023 - Панель управления</title>
...
$(document).ready(function() {
    ...
    function save() {
        ...
        $.ajax({
            type: 'POST',
            url: '/ajax.php?type=update_price_total_2023',
...
<?=menu_dash();?>
           ...
           <br>
           <h2>Тарифы обслуживания 2023</h2>
  ...
  <div class="adm-catalog">
     ...
     <div class="add layout__mb_md">
      ...
      <a style="width: auto;padding-left: 7px;padding-right: 7px;" href="/prices/" class="button">Тариф 2022</a>
