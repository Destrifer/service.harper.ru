<?php
$count = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `prices` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' ;'));
if ($count['COUNT(*)'] > 0) {
$content = mysqli_fetch_array(mysqli_query($db, 'SELECT * FROM `prices` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1;'));
} else {
header('Location: '.$config['url'].'prices/');
}
# Сохраняем:
if ($_POST['send'] == 1) {
...
`acess` = \''.mysqli_real_escape_string($db, $_POST['acess']).'\',
`anrp` = \''.mysqli_real_escape_string($db, $_POST['anrp']).'\',
`ato` = \''.mysqli_real_escape_string($db, $_POST['ato']).'\'
WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1
;') or mysqli_error($db);


admin_log_add('Обновлена цена #'.$_GET['id']);
...
<?=menu_dash();?>
  ...
  <form id="send" method="POST">
   <div class="adm-form" style="padding-top:0;">
                  ...
                  <div class="item">
              <div class="level">Тип техники:</div>
              <div class="value">
                <input type="text" name="type" value="<?=$content['type'];?>"  />
                  ...
                  <div class="item">
              <div class="level">Блочный:</div>
              <div class="value">
                <input type="text" name="block" value="<?=$content['block'];?>"  /> р.
              </div>
            </div>

                    <div class="item">
              <div class="level">Поэлементный:</div>
              <div class="value">
                <input type="text" name="element" value="<?=$content['element'];?>"  /> р.
              </div>
            </div>

                    <div class="item">
              <div class="level">Замена аксессуаров:</div>
              <div class="value">
                <input type="text" name="acess" value="<?=$content['acess'];?>"  /> р.
              </div>
            </div>

                    <div class="item">
              <div class="level">АНРП:</div>
              <div class="value">
                <input type="text" name="anrp" value="<?=$content['anrp'];?>"  /> р.
              </div>
            </div>

                    <div class="item">
              <div class="level">АТО</div>
              <div class="value">
                <input type="text" name="ato" value="<?=$content['ato'];?>"  /> р.
              </div>
            </div>
