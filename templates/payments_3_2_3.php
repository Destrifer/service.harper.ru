...
$sql = mysqli_query($db, 'SELECT `app_date` FROM `repairs` where `service_id` = '.$_GET['service_id'].' and `service_id` != 33 '.$where_date.' and (`status_admin` = \'Подтвержден\' OR `status_admin` = \'Выдан\') and `deleted` = 0 and  `app_date` NOT REGEXP \''.$dated.'\' '.$where_year.'  order by `id` DESC;');
    ...
    //print_r($app);
     //array_reverse($app,true);;
      ...
      if ($summ  && $summ != '-' && $type1['sended'] == 1 && $type1['payed'] != 1) {
      $total += $summ;
 ...
 $rows[] = $content_list;
    unset($content_list);
      ...
      if ($summ  && $summ != '-'  && $type3['sended'] == 1 && $type3['payed'] != 1) {
            $total += $summ;
      $content_list['DT_RowClass'] = $tr;
...
<?=top_menu_admin();?>
 ...
 <?=menu_dash();?>
  ...
  <div class="adm-catalog">
    ...
    top: 70px;
    right: 0px;">
    <a href="#" id="select_all" style="margin-right:10px;">Выбрать все</a>
    </div>

    <form method="POST" id="checkb">
    <table style="">
              <tr>
<td style="padding-left: 20px; "><a style="width: auto;padding-left: 7px;padding-right: 7px;background:#80bd03;color:#fff;vertical-align: middle;" href="#" class="button combine_docs">Сформировать обобщенные документы</a>
