<?php
$count = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `countries` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' ;'));
if ($count['COUNT(*)'] > 0) {
$content = mysqli_fetch_array(mysqli_query($db, 'SELECT * FROM `countries` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1;'));
} else {
header('Location: '.$config['url'].'countires/');
}
# Сохраняем:
if ($_POST['send'] == 1) {

mysqli_query($db, 'UPDATE `countries` SET
`name` = \''.mysqli_real_escape_string($db, $_POST['name']).'\'
WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1
;') or mysqli_error($db);

admin_log_add('Обновлена страна #'.$_GET['id']);

header('Location: '.$config['url'].'countries/');
}
