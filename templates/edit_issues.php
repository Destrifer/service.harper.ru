<?php
$count = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `issues` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' ;'));
if ($count['COUNT(*)'] > 0) {
$content = mysqli_fetch_array(mysqli_query($db, 'SELECT * FROM `issues` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1;'));
} else {
header('Location: '.$config['url'].'issues/');
}
# Сохраняем:
if ($_POST['send'] == 1) {
   ...
   <div class="adm-form" style="padding-top:0;">
              ...
              <div class="value">
                <input type="text" name="name" style="width:400px;" value="<?=$content['name'];?>"  />
                        ...
                        <div class="item">
              <div class="level">Название (EN):</div>
              <div class="value">
                <input type="text" name="eng" style="width:400px;" value="<?=$content['eng'];?>"  />
              </div>
            </div>
