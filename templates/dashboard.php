...
$tabs = UI::getTabs(User::getData('role'));
$statuses = models\ParamsDict::getParamsBySectionID(1);
$masters = Staff::getStaffList(['service_id' => User::getData('service_id'), 'role_id' => 4, 'is_active' => true]);
?>
<!doctype html>
<html>
...
<head>
    ...
    <link href="/_new-codebase/front/components/check-model-service/check-model-service.css?v=<?= VER; ?>" rel="stylesheet" />
    <link href="/_new-codebase/front/components/check-serial/check-serial.css?v=<?= VER; ?>" rel="stylesheet" />
    <style>
        * {
...
<body>
    <?php
    if (User::hasRole('service') && (!empty($tabs['accepted']['cnt']) || !empty($tabs['inprogress']['cnt']))) {
        echo '<div class="top-message top-message_primary">Внимание! Карточка в статусах "Принят" и "В работе" не обрабатывается админом! Срок с момента создания карточки до момента заказа запчасти не должен превышать 3-7 дней (в зависимости от договора). Нарушение сроков дефектовки в статусах "Принят" и "В работе" будем считать нарушением договора. В этом случае вся ответственность за нарушение сроков полностью лежит на вашем сервисе.</div>';
    }
    ?>
    ...
    <div id="user-data" style="display: none"><?= json_encode(['id' => models\User::getData('id'), 'role' => models\User::getData('role')]); ?></div>
    ...
    <script src="/_new-codebase/front/modules/dashboard/js/main.js?<?= intval(microtime(1)) ?>?v=<?= VER; ?>"></script>
    <script src="/_new-codebase/front/components/check-model-service/check-model-service.js?v=<?= VER; ?>"></script>
    <script src="/_new-codebase/front/components/check-serial/check-serial.js?v=<?= VER; ?>"></script>
</body>
