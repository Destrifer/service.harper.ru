...
$content = mysqli_fetch_array(mysqli_query($db, 'SELECT * FROM `requests` WHERE `user_id` = \''.mysqli_real_escape_string($db, User::getData('id')).'\' LIMIT 1;'));
...
if ($_POST['password'] && $_POST['password2']) {
  if ($_POST['password'] == $_POST['password2']) {
      $check_email = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `users` WHERE `email` = \''.mysqli_real_escape_string($db, strip_tags($_POST['email'])).'\';'));
      if ($check_email['COUNT(*)'] == 0) {
          ...
          \''.mysqli_real_escape_string($db, $config['ip']).'\',
          \''.mysqli_real_escape_string($db, date('Y.m.d')).'\',
          \'3\'
          )') or die(mysqli_error($db));

          $user_reg['id'] = mysqli_insert_id($db);
...
mysqli_query($db, 'INSERT INTO `services_link`
          ...
          \''.mysqli_real_escape_string($db, $user_reg['id']).'\',
...
`request` = 1
WHERE `id` = \''.mysqli_real_escape_string($db, User::getData('id')).'\' LIMIT 1
;') or mysqli_error($db);

$mes = '<html>
                      <body bgcolor="#DCEEFC">
                      <h3>Уважаемый '.User::getData('login').'.</h3><br>
...
$mail->addAddress(User::getData('email'));
$mail->isHTML(true);
...
header('Location: '.$config['url'].'my-services/');
 ...
 <?=menu_dash();?>
  ...
  <form id="send" class="validate_form" method="POST">
   <div class="adm-form" style="padding-top:0;">
                  ...
                  <h2>Аккаунт сервисного центра</h2><br>
                    <div class="item">
              <div class="level">Электронная почта:</div>
              <div class="value">
                <input type="text" name="email" value="<?=$content['question'];?>"  />
              </div>
            </div>
              ...
              <div class="value">
                <input type="text" name="password" value="<?=$content['question'];?>"  />
              </div>
            </div>
              ...
              <div class="value">
                <input type="text" data-input-filter="phone" data-intl-tel-input name="tel" value="<?=$content['tel'];?>"  />
              </div>
            </div>

                    <div class="item">
              <div class="level">Пароль еще раз:</div>
              <div class="value">
                <input type="text" name="password2" value="<?=$content['question'];?>"  />
              </div>
            </div>
  ...
  <h2>Анкета сервисного центра</h2><br>
             ...
             <div class="item">
              ...
              <select name="country">
               ...
               <option value="1" <?=($content['country'] == 1) ? 'selected' : '';?>>Россия</option>
               <option value="2" <?=($content['country'] == 2) ? 'selected' : '';?>>Белоруссия</option>
              </select>
              </div>
            </div>
