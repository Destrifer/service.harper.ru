<?php
$count = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `repair_type_brand` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' ;'));
if ($count['COUNT(*)'] > 0) {
$content = mysqli_fetch_array(mysqli_query($db, 'SELECT * FROM `repair_type_brand` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1;'));
} else {
header('Location: '.$config['url'].'repair-types-brand/'.$content['brand_id'].'/');
}
# Сохраняем:
if ($_POST['send'] == 1) {
...
mysqli_query($db, 'UPDATE `repair_type_brand` SET
`name` = \''.mysqli_real_escape_string($db, $_POST['name']).'\'
WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1
;') or mysqli_error($db);


admin_log_add('Обновлен тип ремонта #'.$_GET['id']);

header('Location: '.$config['url'].'repair-types-brand/'.$content['brand_id'].'/');
}


function problems($cat_id = '') {
  global $db;
...
<?=menu_dash();?>
           ...
           <br>
           <h2>Редактирование вида ремонта</h2>
