...
$sql = mysqli_query($db, 'SELECT * FROM `notify_logs` ;');
if (mysqli_num_rows($sql) != false) {
      ...
      <td>'.$row['date'].'</td>
      <td>'.$row['users'].'</td>
      <td><div class="linkz"><a class="t-4" style="margin-left: 6px;" href="/show-notify-log/'.$row['id'].'/"></a></div></td>
      </tr>';
      }
      } else {
      $content_list = '';
  ...
  <table id="table_content" class="display" cellspacing="0" width="100%">
        <thead>
            <tr>
                ...
                <th align="left">Число пользователей</th>
