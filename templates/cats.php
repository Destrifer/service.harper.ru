...
$sql = mysqli_query($db, 'SELECT * FROM `cats` WHERE `is_deleted` = 0;');
if (mysqli_num_rows($sql) != false) {
      ...
      <td>'.$true.'</td>
      <td>'.$true2.'</td>
      <td>'.($row['install_flag'] ? 'Да' : 'Нет').'</td>
      <td align="center" class="linkz" >
      <a class="t-3" title="Редактировать карточку" href="/edit-categories/'.$row['id'].'/" ></a>
      <a class="t-5" title="Удалить карточку" onclick=\'return confirm("Вы уверены, что хотите удалить #'.$row['id'].'?")\'  style="float:right" href="/del-categories/'.$row['id'].'/"></a></td></tr>';
      }
      } else {
...
<?=menu_dash();?>
  ...
  <div class="adm-catalog">
     ...
     <div class="add">
      <a style="width: auto;padding-left: 7px;padding-right: 7px;" href="/add-categories/" class="button">Добавить категорию</a>
    </div>
     <br>
  <table id="table_content" class="display" cellspacing="0" width="100%">
        <thead>
            <tr>
                ...
                <th align="left">Возможен выезд</th>
                <th align="left">Монтаж/демонтаж</th>
