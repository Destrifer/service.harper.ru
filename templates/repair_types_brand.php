...
$sql = mysqli_query($db, 'SELECT * FROM `repair_type_brand` where brand_id = '.$_GET['id'].';');
if (mysqli_num_rows($sql) != false) {
      while ($row = mysqli_fetch_array($sql)) {
      $content_list .= '<tr>
      <td>'.$row['id'].'</td>
      <td>'.$row['name'].'</td>
      <td>'.get_parent($row['parent_id'])['name'].'</td>
      <td align="center" class="linkz" >
      <a class="t-3" title="Редактировать карточку" href="/edit-repair-type-brand/'.$row['id'].'/" ></a>
      <a class="t-5" title="Удалить карточку" onclick=\'return confirm("Вы уверены, что хотите удалить #'.$row['id'].'?")\'  style="float:right" href="/del-repair-type-brand/'.$row['id'].'/"></a></td></tr>';
      }
      } else {
...
function get_parent($cat_id) {
  global $db;
$sql = mysqli_query($db, 'SELECT * FROM `details_problem` where id = '.$cat_id);
...
<?=menu_dash();?>
  ...
  <div class="adm-catalog">
     ...
     <div class="add">
      <a style="width: auto;padding-left: 7px;padding-right: 7px;" href="/add-repair-type-brand/<?=$_GET['id'];?>/" class="button">Добавить вид ремонта</a>
    </div>
     <br>
  <table id="table_content" class="display" cellspacing="0" width="100%">
        <thead>
            <tr>
                ...
                <th align="left">Причина отказа</th>
