...
if (!empty($content['serial']) && !empty($content['model_id'])) {
  ...
  if (in_array($model['brand'], ['HARPER', 'OLTO', 'SKYLINE', 'NESONS']) && !models\Serials::isValid($content['serial'], $content['model_id'])) {
...
<body>
  ...
  <script src='/_new-codebase/front/components/repair-card/repair-card.js?<?= intval(microtime(1)) ?>'></script>
