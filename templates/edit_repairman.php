<?php
$count = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `repairmans` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' and `service_id` = '.\models\User::getData('id').' ;'));
if ($count['COUNT(*)'] > 0) {
$content = mysqli_fetch_array(mysqli_query($db, 'SELECT * FROM `repairmans` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1;'));
} else {
header('Location: '.$config['url'].'repairmans/');
}
# Сохраняем:
if ($_POST['send'] == 1) {

mysqli_query($db, 'UPDATE `repairmans` SET
`name` = \''.mysqli_real_escape_string($db, $_POST['name']).'\',
`surname` = \''.mysqli_real_escape_string($db, $_POST['surname']).'\',
`third_name` = \''.mysqli_real_escape_string($db, $_POST['third_name']).'\'
WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1
;') or mysqli_error($db);


admin_log_add('Обновлен мастер #'.$_GET['id']);
  ...
  <form id="send" method="POST">
   <div class="adm-form" style="padding-top:0;">
          ...
          <div class="item">
              ...
              <div class="value">
                <input type="text" name="third_name" value="<?=$content['third_name'];?>"  />
              </div>
            </div>
