...
$sql = mysqli_query($db, 'SELECT * FROM `cats` where `travel` = 1;');
      while ($row = mysqli_fetch_array($sql)) {
        $count_prices = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `transfer` where `cat_id` = '.$row['id']));
        if ($count_prices['COUNT(*)'] == 0) {
            mysqli_query($db, 'INSERT INTO `transfer` (
            `cat_id`
            ) VALUES (
...
$sql = mysqli_query($db, 'SELECT * FROM `transfer`;');
      while ($row = mysqli_fetch_array($sql)) {
        ...
        if ($count_service['COUNT(*)'] == 0) {
            mysqli_query($db, 'DELETE FROM `transfer` WHERE `id` = '.$row['id'].' LIMIT 1;') or mysqli_error($db);
        }
...
if (mysqli_num_rows($sql) != false) {
      ...
      <td>'.cat($row['cat_id'])['name'].'</td>
          <td><input class="editable" style="width:120px;" data-pass-protect-input readonly type="number" min="0" name="shop" value="'.$row['shop'].'" data-id="'.$row['id'].'" data-service-id="'.$_GET['id'].'"></td>
          <td><input class="editable" style="width:120px;" data-pass-protect-input readonly type="number" min="0" name="buyer" value="'.$row['buyer'].'" data-id="'.$row['id'].'" data-service-id="'.$_GET['id'].'"></td>
          <td><input class="editable" style="width:120px;" data-pass-protect-input readonly type="number" min="0" name="zone_1" value="'.$row['zone_1'].'" data-id="'.$row['id'].'" data-service-id="'.$_GET['id'].'"></td>
          <td><input class="editable" style="width:120px;" data-pass-protect-input readonly type="number" min="0" name="zone_2" value="'.$row['zone_2'].'" data-id="'.$row['id'].'" data-service-id="'.$_GET['id'].'"></td>
          <td><input class="editable" style="width:120px;" data-pass-protect-input readonly type="number" min="0" name="zone_3" value="'.$row['zone_3'].'" data-id="'.$row['id'].'" data-service-id="'.$_GET['id'].'"></td>
          <td><input class="editable" style="width:120px;" data-pass-protect-input readonly type="number" min="0" name="zone_4" value="'.$row['zone_4'].'" data-id="'.$row['id'].'" data-service-id="'.$_GET['id'].'"></td>
...
<head>
<meta charset=utf-8>
<title>Тарифы транспорт 2022 - Панель управления</title>
...
$(document).ready(function() {
    ...
    function save() {
        ...
        $.ajax({
            type: 'POST',
            url: '/ajax.php?type=update_travel_total',
