    ...
    switch (core\App::$URLParams['action']) {
        case 'save-service-form':
            if (!empty($_POST['service_id']) && !empty($_POST['tariff_id'])) {
                models\Tariffs::massChangeTransportTariff($_POST['service_id'], $_POST['tariff_id']);
            }
...
<head>
    <meta charset=utf-8>
    <title>Массово поменять тариф на транспорт - Панель управления</title>
...
<body>
    ...
    <div class="viewport-wrapper">
        ...
        <div class="wrapper">
            ...
            <br>
            <h2>Массово поменять тариф на транспорт</h2>

            <div class="adm-catalog">

                <div class="add">
                    <a style="width: auto;padding-left: 7px;padding-right: 7px;" href="/transfer/" class="button">Тарифы транспорт</a>
                </div>
                <br>
                ...
                <form action="?action=save-service-form" method="POST">
                    <h3 style="font-size: 21px;font-weight: 300;margin: 32px 0">Показать:
                        ...
                        echo (!$t) ? '<b>все</b>' : '<a href="/mass-transport-tariff-change/">все</a>';
                        echo ' / ';
...
function getServicesHTML()
{
    ...
    while ($row = mysqli_fetch_assoc($sql)) {
        ...
        $html .= '<label class="service-row"><input data-check-flag="" type="checkbox" name="service_id[]" value="' . $row['user_id'] . '"> ' . $row['name'] . ' <br><span class="service-tariff" title="Текущий тариф">' . $tariffs[$row['transport_tariff_id']] . '</span></label>';
        $n++;
    }
    $html .= '</div>';
