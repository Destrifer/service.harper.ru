...
mysqli_query($db, 'INSERT INTO `repairmans` (
`name`,
`surname`,
`third_name`,
`service_id`
...
\''.mysqli_real_escape_string($db, $_POST['third_name']).'\',
\''.mysqli_real_escape_string($db, User::getData('id')).'\'
);') or mysqli_error($db);

admin_log_add('Добавлен новый мастер '.$_POST['surname'].' '.$_POST['name']);

header('Location: /repairmans/');
}
