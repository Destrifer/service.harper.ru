...
$sql = mysqli_query($db, 'SELECT * FROM `parts_sc` where `service_id` = '.\models\User::getData('id'));
...
$sql2 = mysqli_query($db, 'SELECT * FROM `parts` where `id` = '.$row['part_id'].';');
      ...
      $parent_names = get_parents($row2['id']);
      $content_list .= '<tr>
      <td >'.$row['date_get'].'</td>
      <td >'.$row['date_use'].'</td>
      <td>'.cat_by_id($row2['cat'])['name'].'</td>
      <td >'.$model.'</td>
      <td>'.$row2['serial'].'</td>
       <td>'.$row2['codepre'].$row2['id'].'</td>
      <td>'.$row2['part'].'</td>
      <td>'.$row2['list'].'</td>
      <td>'.$row['count'].'</td><td><a href="https://crm.r97.ru/edit-repair/'.$row['repair_id'].'/">'.$row['repair_id'].'</td></tr>';
      }
   ...
   <table id="table_content" class="display" cellspacing="0" width="100%">
        <thead>
            <tr>
                ...
                <th align="left" data-priority="5">Партномер</th>
                <th align="left" data-priority="6">Наименование запчасти</th>
                <th align="left" data-priority="7">Количество</th>
