<?php
$count = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `cityfull` WHERE `fcity_id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' ;'));
if ($count['COUNT(*)'] > 0) {
$content = mysqli_fetch_array(mysqli_query($db, 'SELECT * FROM `cityfull` WHERE `fcity_id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1;'));
} else {
...
mysqli_query($db, 'UPDATE `cityfull` SET
`fcity_name` = \''.mysqli_real_escape_string($db, $_POST['fcity_name']).'\',
`fcity_country` = \''.mysqli_real_escape_string($db, $_POST['fcity_country']).'\'
WHERE `fcity_id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1
;') or mysqli_error($db);

admin_log_add('Обновлен город #'.$_GET['id']);  
  ...
  <form id="send" method="POST">
   <div class="adm-form" style="padding-top:0;">
             ...
             <div class="item">
              ...
              <div class="value">
                <input type="text" name="fcity_name" value="<?=$content['fcity_name'];?>"  />
              </div>
            </div>
