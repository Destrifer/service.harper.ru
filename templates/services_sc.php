...
$sql = mysqli_query($db, 'SELECT * FROM `services_link` where `service_parent` = '.\models\User::getData('id').';');
      if (mysqli_num_rows($sql) > 0) {
      while ($row2 = mysqli_fetch_array($sql)) {
      $row = Users::getUser(['id' => $row2['service_child']]);
      $info = get_request_info($row['id']);
      $city = get_city($info['city']);

      $status = ($row['mod'] == 0) ? '<span style="color:red;">На проверке</span>' : '<span style="color:green;">Одобрен</span>';

      $block_style = ($row['block'] == 0) ? '' : 'style="background: rgba(255, 71, 71, 0.13);"';
      $content_list .= '<tr '.$block_style.'><td '.$block_style.'>'.$row['id'].'</td><td>'.$info['name'].'</td><td>'.$row['email'].'</td><td>'.$info['phones'].'</td><td>'.$city['fcity_name'].'</td><td>'.$info['phisical_adress'].'</td><td>'.$status.'</td>
      <td align="center" class="linkz"><a class="t-3" href="/my-service/'.$row['id'].'/edit/" title="Редактировать" ></a></td>
      </tr>';
 ...
 <?=menu_dash();?>
  ...
  <div class="adm-catalog">
     ...
     <div class="add" style="padding-top:0px;">
      <a style="width: auto;padding-left: 7px;padding-right: 7px;" href="/add-my-service/" class="button">Добавить новый СЦ</a>
    </div>  <br>
  ...
  <table id="table_content" class="display" cellspacing="0" width="100%">
        <thead>
            <tr>
                ...
                <th align="left" data-priority="3">Email</th>
                <th align="left" data-priority="3">Телефон</th>
                <th align="left" data-priority="3">Город</th>
