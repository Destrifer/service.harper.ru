    ...
    switch (core\App::$URLParams['action']) {
        case 'save-service-form':
            if (!empty($_POST['service_id'])) {
                models\Transport::sychTariff($_POST['service_id']);
            }
...
<head>
    <meta charset=utf-8>
    <title>Синхронизировать тарифы транспорта - Панель управления</title>
...
<body>
    ...
    <div class="viewport-wrapper">
        ...
        <div class="wrapper">
            ...
            <br>
            <h2>Синхронизировать тарифы транспорта</h2>

            <div class="adm-catalog">

                <div class="add">
                    <a style="width: auto;padding-left: 7px;padding-right: 7px;" href="/transfer/" class="button">Тарифы</a>
                </div>
                <br>
...
function getServicesHTML()
{
    ...
    $sql = mysqli_query($db, 'SELECT r.`user_id`, r.`name` 
    FROM `requests` r 
    LEFT JOIN `users` u ON u.`id` = r.`user_id` 
    WHERE r.`mod` = 1 AND u.`role_id` = 3 
    AND u.`status_id` = 1 ORDER BY r.`name`;');
    $n = 0;
