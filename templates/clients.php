...
if (mysqli_num_rows($sql) != false) {
      ...
      <a class="t-3" title="Редактировать карточку" href="/edit-client/'.$row['id'].'/" ></a>
      <a class="t-5" title="Удалить карточку" onclick=\'return confirm("Вы уверены, что хотите удалить #'.$row['id'].'?")\'  style="float:right" href="/del-client/'.$row['id'].'/"></a></td></tr>';
      }
      } else {
      $content_list = '<tr><td colspan="7">'.$row['id'].'</td>';
      }
    return $content_list;
}
}
...
<?=menu_dash();?>
           ...
           <br>
           <h2>Клиенты (юр.лица)</h2>

  <div class="adm-catalog">

     <div class="add">
      <a style="width: auto;padding-left: 7px;padding-right: 7px;" href="/add-client/" class="button">Добавить клиента</a>
    </div>
     <br>
  <table id="table_content" class="display" cellspacing="0" width="100%">
        <thead>
            <tr>
                ...
                <th align="left">Ведущий менеджер</th>
                <th align="left">Контакт менеджера</th>
