...
$sql = mysqli_query($db, 'SELECT `app_date` FROM `repairs` where `service_id` = '.$_GET['service_id'].' and `service_id` != 33 '.$where_date.' and (`status_admin` = \'Подтвержден\' OR `status_admin` = \'Выдан\') and `deleted` = 0 and  `app_date` NOT REGEXP \''.$dated.'\' '.$where_year.'  order by `id` DESC;');
    ...
    //print_r($app);
     //array_reverse($app,true);;
      ...
      $type13 = create_or_get_payment_id(User::getData('id'), $year, $month, 13); //акт
      $type14 = create_or_get_payment_id(User::getData('id'), $year, $month, 14); //счет
      }
 ...
 $rows[] = $content_list;
    unset($content_list);
      ...
      $content_list[] = $year.'.'.$month.'<br><u>TESLER</u><br><br>'.(($tr) ? '<img src="/img/skull.png">' : '');
 ...
 $rows[] = $content_list;
    unset($content_list);
      ...
      $tr = (get_service_loan($_GET['service_id'], $month, $year, 13, 14)) ? ' impor ' : '';
      ...
      $content_list[] = $year.'.'.$month.'<br><u>SELENGA</u><br><br>'.(($tr) ? '<img src="/img/skull.png">' : '');
      $content_list[] = '<a href="/get-payment-act-admin/'.$_GET['service_id'].'/'.$year.'/'.$month.'/selenga-brand/">Акт оплаченных работ</a>';
      $content_list[] = '<a href="/get-payment-bill-admin/'.$_GET['service_id'].'/'.$year.'/'.$month.'/selenga-brand/">Счет на оплату</a>';
      $content_list[] = $summ.',00 руб.';
      $content_list[] = '<table style="    margin: 0 auto;" class="nohov min_width"><tr><td>
      <form method="POST">
      ...
      $content_list[] = '<div class="linkz"><a class="t-1" href="/get-payment-archive-admin/'.$_GET['service_id'].'/'.$year.'/'.$month.'/selenga/" title="Скачать архивом" ></a>'.(check_combined($type13['id'], $type14['id']) ? '<br><br><input type="checkbox" value="1" name="combine" data-act-id="'.$type13['id'].'" data-bill-id="'.$type14['id'].'">' : '').'</div>';
      $content_list[] = $year.'.'.$month;
 $rows[] = $content_list;
    unset($content_list);
      }
      ...
      $content_list[] = $year.'.'.$month.'<br><u>HARPER</u><br><br>'.(($tr) ? '<img src="/img/skull.png">' : '');
      $content_list[] = '<a href="/get-payment-act-admin/'.$_GET['service_id'].'/'.$year.'/'.$month.'/">Акт оплаченных работ</a>';
      $content_list[] = '<a href="/get-payment-bill-admin/'.$_GET['service_id'].'/'.$year.'/'.$month.'/">Счет на оплату</a>';
      $content_list[] = $summ.',00 руб.';
      $content_list[] = '<table style="    margin: 0 auto;" class="nohov min_width"><tr><td>
...
$content_list['DT_RowClass'] = $tr;
      ...
      $content_list[] = $year.'.'.$month.'<br><u>TESLER</u>'.(($tr) ? '<img src="/img/skull.png">' : '');
      $content_list[] = $req['name'];
...
$content_list['DT_RowClass'] = $tr;
 ...
 $rows[] = $content_list;
    unset($content_list);
      ...
      if ($selenga == 1) {
      $tr = (get_service_loan($row2['service_id'], $month, $year, 13, 14)) ? 'impor' : '';
      if ($type14['status'] == 0) {
      $status2 = '<td style="background: rgba(255, 71, 71, 0.13);">Не оплачено</td>';
      } else {
      $status2 = '<td style="background: rgba(116, 220, 116, 0.13);">Оплачено</td>';
...
$content_list['DT_RowClass'] = $tr;
      ...
      $content_list[] = $year.'.'.$month.'<br><u>SELENGA</u>'.(($tr) ? '<img src="/img/skull.png">' : '');
      $content_list[] = $req['name'];
      $content_list[] = '<a href="/get-payment-act-admin/'.$row2['service_id'].'/'.$year.'/'.$month.'/selenga-brand/">Акт оплаченных работ</a>';
      $content_list[] = '<a href="/get-payment-bill-admin/'.$row2['service_id'].'/'.$year.'/'.$month.'/selenga-brand/">Счет на оплату</a>';
      $content_list[] = $summ.',00 руб.';
      $content_list[] = '<table style="    margin: 0 auto;" class="nohov min_width"><tr><td>
      <form method="POST">
      <select  name="status_pay" data-pay-id="'.$type14['id'].'">
      ...
      $content_list[] = '<div class="linkz"><a class="t-1" href="/get-payment-archive-admin/'.$row2['service_id'].'/'.$year.'/'.$month.'/selenga/" title="Скачать архивом" ></a>'.(check_combined($type13['id'], $type14['id']) ? '<br><br><input type="checkbox" value="1" name="combine" data-act-id="'.$type13['id'].'" data-bill-id="'.$type14['id'].'">' : '').'</div>';
      $content_list[] = $year.'.'.$month;
 $rows[] = $content_list;
    unset($content_list);
      }
...
$content_list['DT_RowClass'] = $tr;
 ...
 $rows[] = $content_list;
   ...
   $tr = (get_service_loan($row2['service_id'], $month, $year, 13, 14)  || check_custom_loan($row2['service_id'])) ? ' impor' : '';
...
<?=top_menu_admin();?>
 ...
 <?=menu_dash();?>
  ...
  <div class="adm-catalog">
    ...
    <div style="display:inline-block;"><span style="width: 70px;display: inline-block;text-align:right;">Сервис </span>&nbsp;&nbsp;<select class="select2 nomenu" name="service_id"><option value="all">Все сервисы</option><?=services_select($_GET['service_id']);?></select><input class="green_button" type="submit" style="display: inline-block;margin-left:15px;  vertical-align: middle;    height: 40px;    margin-top: -4px;" value="Применить" /></div>
    </form>
    </div>
    <hr> <br>
    ...
    <table style="">
              ...
              <td><div class="add" style="padding-top:0px;display:inline-block;padding-right:5px;">

     <a style="width: auto;padding-left: 7px;padding-right: 7px;background:#80bd03;color:#fff;vertical-align: middle;" href="/combined/" class="button">Архив документов</a>
    </div> <div class="add" style="padding-top:0px;display:inline-block;">
