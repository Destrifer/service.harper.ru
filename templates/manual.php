<?php
$count = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `configuration` WHERE `id` = 12 ;'));
if ($count['COUNT(*)'] > 0) {
$content = mysqli_fetch_array(mysqli_query($db, 'SELECT * FROM `configuration` WHERE `id` = 12 LIMIT 1;'));
} else {
...
<?=menu_dash();?>
           ...
           <br>
           <h2>Инструкция по работе с базой</h2>
