...
$sql = mysqli_query($db, 'SELECT * FROM `repairs` where `service_id` = '.$_GET['service_id'].' '.$where_date.' and `status_admin` = \'Подтвержден\' and `deleted` = 0 and  `app_date` NOT REGEXP \''.$dated.'\' '.$where_year.'  order by `id` DESC;');
    ...
    //print_r($app);
     //array_reverse($app,true);;
      ...
      <td align="center" class="linkz"><a class="t-1" href="/get-payment-archive-admin/'.$_GET['service_id'].'/'.$year.'/'.$month.'/tesler/" title="Скачать архивом" ></a>'.(check_combined($type3['id'], $type4['id']) ? '<br><br><input type="checkbox" value="1" name="combine" data-act-id="'.$type3['id'].'" data-bill-id="'.$type4['id'].'">' : '').'</td>
      </tr>';
      }
      }
      }
      ...
      <td align="center" class="linkz"><a class="t-1" href="/get-payment-archive-admin/'.$_GET['service_id'].'/'.$year.'/'.$month.'/" title="Скачать архивом" ></a>'.(check_combined($type1['id'], $type2['id']) ? '<br><br><input type="checkbox" value="1" name="combine" data-act-id="'.$type1['id'].'" data-bill-id="'.$type2['id'].'">' : '').'</td>
      </tr>';
      }
...
$sql = mysqli_query($db, 'SELECT * FROM `repairs` where `app_date` != \'\' '.$where_date.' '.$where_year.' and `status_admin` = \'Подтвержден\' and `deleted` = 0 order by `id` DESC;');
  ...
  //print_r($type1);
      ...
      <td align="center" class="linkz"><a class="t-1" href="/get-payment-archive-admin/'.$row2['service_id'].'/'.$year.'/'.$month.'/" title="Скачать архивом" ></a>'.(check_combined($type1['id'], $type2['id']) ? '<br><br><input type="checkbox" value="1" name="combine" data-act-id="'.$type1['id'].'" data-bill-id="'.$type2['id'].'">' : '').'</td>
      </tr>';
      }
      ...
      <td align="center" class="linkz"><a class="t-1" href="/get-payment-archive-admin/'.$row2['service_id'].'/'.$year.'/'.$month.'/tesler/" title="Скачать архивом" ></a>'.(check_combined($type3['id'], $type4['id']) ? '<br><br><input type="checkbox" value="1" name="combine" data-act-id="'.$type3['id'].'" data-bill-id="'.$type4['id'].'">' : '').'</td>
      </tr>';
      }
      }
