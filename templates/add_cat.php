...
//echo get_service_summ_fast('33', '10', '2018', 'TESLER'); 
...
mysqli_query($db, 'INSERT INTO `cats` (
`name`,
`service`,
`warranty`,
`travel`,
...
\''.mysqli_real_escape_string($db, $_POST['warranty']).'\',
\''.mysqli_real_escape_string($db, $_POST['travel']).'\',
\''.mysqli_real_escape_string($db, $_POST['install_flag']).'\'
);') or mysqli_error($db);

admin_log_add('Добавлена новая категория '.$_POST['name']);
  ...
  <form id="send" method="POST">
   <div class="adm-form" style="padding-top:0;">
            ...
            <div class="item">
              ...
              <div class="value">
                <input type="text" name="warranty" value="365"  />
              </div>
            </div>

                         <div class="adm-finish" style="padding-top:0px;">
            <ul>
              <li><label><input type="checkbox" name="service" value="1"/>Обслуживается</label></li>
              <li><label><input type="checkbox" name="travel" value="1"/>Возможен выезд к клиенту</label></li>
              <li><label><input type="checkbox" name="install_flag" value="1"/>Возможен монтаж/демонтаж</label></li>
            </ul>
            </div>
