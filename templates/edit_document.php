<?php
$count = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `documents` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' ;'));
if ($count['COUNT(*)'] > 0) {
$content = mysqli_fetch_array(mysqli_query($db, 'SELECT * FROM `documents` WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1;'));
} else {
...
mysqli_query($db, 'UPDATE `documents` SET
`name` = \''.mysqli_real_escape_string($db, $_POST['name']).'\'
WHERE `id` = \''.mysqli_real_escape_string($db, $_GET['id']).'\' LIMIT 1
;') or mysqli_error($db);

admin_log_add('Обновлен документ #'.$_GET['id']);

header('Location: '.$config['url'].'documents/'.$_GET['sc'].'/'); 
}
