...
$sql = mysqli_query($db, 'SELECT * FROM `issues` ;');
if (mysqli_num_rows($sql) != false) {
      while ($row = mysqli_fetch_array($sql)) {
      if ($row['used'] == '') {
      $check_repairs = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `repairs` where `disease` = '.$row['id'].' ;'));

      if ($check_repairs['COUNT(*)'] > 0) {
      mysqli_query($db, 'UPDATE `issues` set `used` = 1 where `id` = '.$row['id']);
      } else {
      mysqli_query($db, 'UPDATE `issues` set `used` = 0 where `id` = '.$row['id']);  
      }

      } else {
      $answer = ($row['used'] == 1) ? 'Да' : 'Нет';
      }
      $content_list .= '<tr>
      <td>'.$row['id'].'</td>
      <td>'.$row['name'].'</td>
      ...
      <a class="t-3" title="Редактировать карточку" href="/edit-issue/'.$row['id'].'/" ></a>
      <a class="t-5" title="Удалить карточку" onclick=\'return confirm("Вы уверены, что хотите удалить #'.$row['id'].'?")\'  style="float:right" href="/del-issue/'.$row['id'].'/"></a></td></tr>';
      }
      } else {
...
<?=menu_dash();?>
           ...
           <br>
           <h2>Фактическая неисправность</h2>

  <div class="adm-catalog">

       <div class="add">
      <a style="width: auto;padding-left: 7px;padding-right: 7px;" href="/add-issue/" class="button">Добавить симптом</a>
    </div>
     <br>
  <table id="table_content" class="display" cellspacing="0" width="100%">
        <thead>
            <tr>
                <th align="left">#</th>
                <th align="left">Симптом</th>
                <th align="left">Используется</th>
