...
$sql = mysqli_query($db, 'SELECT * FROM `cats` where `service` = 1;');
      while ($row = mysqli_fetch_array($sql)) {
        $count_prices = mysqli_fetch_array(mysqli_query($db, 'SELECT COUNT(*) FROM `prices_2` where `cat_id` = '.$row['id']));
        if ($count_prices['COUNT(*)'] == 0) {
            mysqli_query($db, 'INSERT INTO `prices_2` (
            `cat_id`
            ) VALUES (
...
$sql = mysqli_query($db, 'SELECT * FROM `prices_2`;');
      while ($row = mysqli_fetch_array($sql)) {
        ...
        if ($count_service['COUNT(*)'] == 0) {
            mysqli_query($db, 'DELETE FROM `prices_2` WHERE `id` = '.$row['id'].' LIMIT 1;') or mysqli_error($db);
        }
...
$(document).ready(function() {
    ...
    function save() {
        ...
        $.ajax({
            type: 'POST',
            url: '/ajax.php?type=update_price_total2',
