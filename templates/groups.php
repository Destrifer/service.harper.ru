...
$sql = mysqli_query($db, 'SELECT * FROM `parts2_groups` ORDER BY `name`');
if (mysqli_num_rows($sql) != false) {
      ...
      <a class="t-3" title="Редактировать карточку" href="/edit-group/'.$row['id'].'/" ></a>
      <a class="t-4" title="Копировать карточку" href="/copy-group/'.$row['id'].'/" ></a>
      <a class="t-5" title="Удалить карточку" onclick=\'return confirm("Вы уверены, что хотите удалить #'.$row['id'].'?")\'  style="float:right" href="/del-group/'.$row['id'].'/"></a></td></tr>';
      }
      } else {
...
<head>
<meta charset=utf-8>
<title>Группы запчастей - Панель управления</title>
...
<?=menu_dash();?>
  ...
  <div class="adm-catalog">
     ...
     <div class="add">
      <a style="width: auto;padding-left: 7px;padding-right: 7px;" href="/add-group/" class="button">Добавить группу</a>
    </div>
     <br>
